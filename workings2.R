...
df <- read_xml("timelines.xml")
df_list <- as_list(df)
xml_find_all(df, ".//statisticians")
df <- xmlParse("timelines.xml")
xmlToDataFrame(df, nodes=getNodeSet(df, "//statisticians"))
xmlToDataFrame(df, nodes=getNodeSet(df, "//tasks"))
xmlToDataFrame(df, nodes=getNodeSet(df, "//studies"))
xmlToDataFrame(df, nodes=getNodeSet(df, "//timelines"))

df|> select(-ID) |> rename("ID"="task")|>
  left_join(tasks)|>
